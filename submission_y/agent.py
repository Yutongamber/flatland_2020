...
class Agent:
    ...
    def load(self, path,  *defaults):
        try:
            ...
            print('model_checkpoint.policy')
            self.policy.load_state_dict(torch.load('ppo/model_checkpoint.policy'))
            self.optimizer.load_state_dict(torch.load('ppo/model_checkpoint.optimizer'))
            print("L")
            with open('ppo/model_checkpoint.meta', 'rb') as file:
                print("LL")
