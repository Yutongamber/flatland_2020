...
class DDDQNPolicy(Policy):
    ...
    def load(self, filename):
        if os.path.exists(filename + ".local"):
            self.qnetwork_local.load_state_dict(torch.load(filename + ".local", map_location=torch.device('cpu')))
            print('local')
        if os.path.exists(filename + ".target"):
            self.qnetwork_target.load_state_dict(torch.load(filename + ".target", map_location=torch.device('cpu')))
            print('target')
