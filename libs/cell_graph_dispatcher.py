...
time_coef = -0.9704545904
speed_coef = -3035.858237
...
class CellGraphDispatcher:
    def __init__(self, env: RailEnv, sort_function=None):
        ...
        # controllers里面是多个CellGraphAgent类
        self.controllers = [CellGraphAgent(agent, self.graph, self.locker, i, max_steps) for i, agent in enumerate(env.agents)]
        # 长度为agent的数量
        self.action_dict = {}

        if sort_function is None:
            # sort_function = lambda idx: self.controllers[idx].dist_to_target[self.graph._vertex_idx_from_point(env.agents[idx].initial_position), env.agents[idx].initial_direction] \
            #                                        - 10000 * env.agents[idx].speed_data['speed']
            sort_function = lambda idx: time_coef * self.controllers[idx].dist_to_target[self.graph._vertex_idx_from_point(env.agents[idx].initial_position), env.agents[idx].initial_direction] \
                                                   + speed_coef * env.agents[idx].speed_data['speed']
            # 希望越快的车和距离越短的车优先发车
        else:
            sort_function = sort_function(self)

        self.agents_order = sorted(range(len(env.agents)), key=sort_function) # 按照sort_function的规则排序
        self.agent_locked_by_malfunction = []  # 布尔变量
    ...
    def step(self, step) -> Dict[int, RailEnvActions] :
        try:
            ...
            for i, agent in enumerate(self.env.agents):
                is_locked = agent.malfunction_data['malfunction']  # 只为0或1
            ...
            updated = set()       # 每一次step初始化updated

            full_recalc_needed = False
            # old_locker = None
            try:
                if has_new_malfunctions:  # 如果出现了新的malfunc则进行重新规划
                    ...
                    cached_ways, vertex_agent_order, agent_way_position, agent_position_duration = \
                        cell_graph_rescheduling_data.get_rescheduling_data(self.env, step, self.controllers, self.graph, self.locker)
                    ...
                    new_way, full_recalc_needed = cell_graph_rescheduling.reschedule(cached_ways, vertex_agent_order, agent_way_position, agent_position_duration,
                                                                                     self.env, step, self.controllers, self.graph, self.locker)
                    # 返回的new_way里面每一个元素是一个AgentWayStep类
                    for i in self.agents_order:
                        if len(new_way[i]):
                            changed = cell_graph_rescheduling.recover_agent_way(self.controllers[i], self.env.agents[i], self.graph, new_way[i])
                            if changed:
                                updated.add(i)
            # resheduling failed, try to make a partial rescheduling
                ...
                self.partial_resheduling(cached_ways, vertex_agent_order2, agent_way_position2, agent_position_duration2, step)
                self.limit_max_visited()

            for i in self.agents_order:         # 如果没有mulfunc，则根据顺序来进行调度
                try:
                    ...
                    force_new_path = full_recalc_needed or self.crashed or i in updated  # 正常情况下为False
                    ...
                    #     # continue
                    if i in self.blocked_agents:     # 堵住了就需要规划新路线
                        force_new_path = True
    ...
    def partial_resheduling(self, cached_ways, vertex_agent_order2, agent_way_position2, agent_position_duration2, step):
        print('partial_resheduling')
        try:
            new_way, blocked_agents = cell_graph_partial_rescheduling.partial_reschedule(cached_ways, vertex_agent_order2, agent_way_position2,
                                                             agent_position_duration2,
                                                               self.env, step, self.controllers, self.graph,
                                                               self.locker)

            for i in self.agents_order:
                if len(new_way[i]):
                    cell_graph_rescheduling.recover_agent_way(self.controllers[i], self.env.agents[i], self.graph, new_way[i])
