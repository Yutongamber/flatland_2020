...
class CellGraphLocker:
    def __init__(self, graph):
        self.graph = graph
        self.data = []   # list of list of list，长度为vertex数量，每一个vertex中包含按照先后顺序排列好的(duration, agent_idx)
        self.reset()


    def reset(self):
        vertexes = len(self.graph.vertexes)
        self.data = [[] for i in range(vertexes)]   # list of list


    def lock(self, vertex_idx, agent_idx, duration):       # 每一次lock都更新self.data
        ...
        index = self.equal_or_greater_index_end(vertex_idx, duration[1])  # duration[0]是到达时间，1是从这个点的出发时间
    ...
    def _has_conflict(self, left, right):          # left和right都是[duration,agent_index]
        if (left is None) or (right is None):
            return False

        d1 = left[0]   # 左边duration
        d2 = right[0]   # 右边duration

        if left[1] > right[1]:   
            d1 = (d1[0], d1[1] + 1)

        return self._has_intersection(d1, d2)   # d1和d2要是有交集则会冲突
